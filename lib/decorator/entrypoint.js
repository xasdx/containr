...
export default (containr) => {
  return (entryPointMethod) => {
    ...
    if (!entryPointMethod || typeof entryPointMethod !== "string") {
      throw Error("EntryPoint must receive an argument, pointing to a method used as entry point")
    }
    ...
    return (target, name, descriptor) => {
      ...
      if (containr.entryPoint !== null) {
        throw Error(`EntryPoint has already been defined as ${containr.entryPoint.component}::${containr.entryPoint.method}`)
      }
